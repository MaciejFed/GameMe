...
export function animateRobot(stage, robotBitmap, arrowBitmap, diamonds, road){
    ...
    const robotPositions = toMapPositions({ x: robotBitmap.x, y: robotBitmap.y }, road);
    const arrowPositions = toMapPositions({ x: arrowBitmap.x, y: arrowBitmap.y }, road);
    ...
    for (let i = 1; i <= road.length; i++) {
        if(road[i-1].collectedDiamonds.length === 0){
            ...
            arrowTween.to({ x: arrowPositions[i].x, y: arrowPositions[i].y }, ONE_STEP_TIME, createjs.Ease.getPowInOut(4));
        }
